...
export const featuresTabData = [
  ...
  { id: "map", name: "Map & Directions", icon: MapPin },
];
...
const TourFeaturesTab = () => {
  ...
  return (
    <div className="my-12 md:my-24">
      ...
      {/* SAFE TAB CONTENT AREA - No conditional mounting */}
      <div className="my-8">
        {/* ACCOMMODATION TAB */}
        <div hidden={tabIndex !== "accommodation"}>
          <AccommodationTab data={data.accommodation} slug={data.slug} />
        </div>

        {/* DINING TAB */}
        <div hidden={tabIndex !== "dining"}>
          <DiningTab dinings={data.dinings} />
        </div>

        {/* WELLNESS TAB */}
        <div hidden={tabIndex !== "wellness"}>
          <WellnessTab wellnessSpa={data.wellnessSpa} />
        </div>

        {/* ACTIVITIES TAB */}
        <div hidden={tabIndex !== "activities"}>
          <ActivitiesTab activities={data.activities} />
        </div>

        {/* MAP TAB */}
        <div hidden={tabIndex !== "map"}>
          <MapTab data={data.direction} />
        </div>
      </div>
    </div>
  );
