...
const HighlightTab = ({ highlightedList, map, highlightContent, isRoomDetails = false }: HighlightTabProps) => {
    if (!highlightedList) return null
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center mb-6">
            <div className='space-y-4'>
                <h3 className='text-xl font-medium font-work-sans'>{highlightContent.title}</h3>
                <div className='space-y-5'>
                    {
                        highlightContent.highlights.map((item, index) => (
                            <p key={index} className='text-gray-500 text-base'>{item}</p>
                        ))
                    }
                </div>
            </div>
