...
const SriLankaDestination = () => {
  ...
  const slPackages = tourPackagesCardData.filter(
    (x) => x.country === "sri-lanka"
  );
      ...
      <div className="flex justify-center">
        <div className="relative flex bg-white shadow-sm rounded-full py-2 gap-3">
          ...
          <button
            ...
            className={`relative cursor-pointer z-10 px-8 py-3 text-lg font-medium transition-colors ${
              activeTab === "overview" ? "text-white" : "text-[#4A4A4A]"
            }`}
          >
          ...
          <button
            ...
            className={`relative cursor-pointer z-10 px-8 py-3 text-lg font-medium transition-colors ${
              activeTab === "getaways" ? "text-white" : "text-[#4A4A4A]"
            }`}
          >
            <span className="flex items-center gap-x-3 text-base uppercase">
              <LuPlane /> Getaways
      ...
      <div className="mt-6">
        <AnimatePresence mode="wait">
          ...
          {activeTab === "getaways" && (
            <motion.div
              key="getaways"
              initial={{ opacity: 0, x: -40 }} // enter from left
              animate={{ opacity: 1, x: 0 }} // move to center
              exit={{ opacity: 0, x: 40 }} // exit to right
